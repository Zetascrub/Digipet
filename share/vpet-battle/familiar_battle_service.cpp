...
void FamiliarBattleService::handleIncomingMessage(const uint8_t* data,
    ...
    switch (data[0]) {
        ...
        case kMsgCapabilities: {
            ...
            if (active) addLog("Enhanced battle capabilities negotiated.");
            break;
        }
        case kMsgGenomeChunk: {
            ...
            if (opponentGenomeCodeAvailable()) {
                ...
                addLog("Opponent genome available to copy.");
            }
            break;
        }
        default:
...
void FamiliarBattleService::logAction(bool actorIsMe, const BattleRulesActionResult& action) {
    ...
    if (action.missed) {
        addLog(String(actorIsMe ? "Your" : "Opponent's") + " Special missed!");
        return;
    }
    addLog(String(actorIsMe ? "You" : "Opponent") + " used " +
          (action.special ? "Special" : "Attack") + " for " + String(action.damage) +
          " dmg.");
}
